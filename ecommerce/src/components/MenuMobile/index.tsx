...
export default function MenuMobile() {
  ...
  const optionsMenu = [
    ...
    { href: '/clube', text: 'Clube' },
    { href: '/loja', text: 'Loja' },
    { href: '/produtores', text: 'Produtores' },
      ...
      <nav className='w-full  flex-col'>
        <menu className=" flex-col ">
          {optionsMenu.map((item, index) => (
            <li key={index} className=" w-full h-20 flex items-center justify-center border-b border-[#555555]">
              <a className='text-center text-[#B6116E]' href={item.href}> {item.text}</a>
            </li>
          ))}
        </menu>
      </nav>
  ...
  return (
    <div className='md:hidden flex items-center'>
      <Button data-testid='openButton' onClick={toggleDrawer('left', true)}>
